...
def read_csv_table(csvFile):
    ...
    """
    table = []
    with open(csvFile, 'rt', encoding='utf-8-sig') as file_in: #encoding='utf-8-sig' would remove \ufeff in the beginning of Windows UTF-8 files
        csv_reader = csv.reader(file_in, delimiter=',', quoting=csv.QUOTE_ALL)
...
def write_table_csv(exportFile, table):
    with open(exportFile, 'wt') as csvfile:
        textWriter = csv.writer(csvfile, delimiter=',', quoting=csv.QUOTE_ALL)
        for i in range(len(table)):
